export const glossary: Record<string, string> = {
  ...
  'dx-gateway': 'Direct Connect Gateway — an AWS resource that connects a Virtual Private Gateway (or Transit Gateway) to a Direct Connect or Interconnect link. Created by Terraform in this demo.',
  ...
  interconnect: 'AWS Interconnect – multicloud — a managed private connectivity service that connects Amazon VPCs directly to VPCs on other cloud providers. GA with Google Cloud as of April 2026.',
  macsec: 'IEEE 802.1AE MACsec — encryption applied to the physical links between AWS and GCP routers at interconnection facilities. Enabled by default on every Interconnect connection.',
  'partner-cci': 'Partner Cross-Cloud Interconnect — the GCP-side service that accepts the AWS activation key and creates a managed transport for cross-cloud connectivity.',
  ...
  terraform: 'HashiCorp Terraform — infrastructure-as-code tool used to provision the base AWS and GCP resources in this demo.',
  transport: 'GCP Transport — a Network Connectivity resource created via `gcloud beta network-connectivity transports create`. Represents the GCP side of the cross-cloud interconnect link.',
  ...
  'vpc-peering': 'VPC Network Peering — a GCP feature that connects two VPC networks for private route exchange. Used here to peer the workload VPC with the transport managed network.',
};
